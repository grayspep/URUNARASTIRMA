...
// Migrasyon: ai_analiz sütunu yoksa ekle
try {
  db.exec(`ALTER TABLE urunler ADD COLUMN ai_analiz TEXT;`);
  db.exec(`ALTER TABLE urunler ADD COLUMN ai_tarih DATETIME;`);
} catch(e) {}

// Varsayılan kullanıcılar
const kc = db.prepare("SELECT COUNT(*) as c FROM kullanicilar").get().c;
if (kc === 0) {
  const ins = db.prepare("INSERT INTO kullanicilar (isim, renk) VALUES (?,?)");
  ins.run("Egemen",          "#4f7cff");
  ins.run("Tuna",            "#22c55e");
  ins.run("Arda",            "#f59e0b");
  ins.run("Melisa",          "#a78bfa");
...
app.post("/api/urunler", (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO urunler (urun,link,gorsel,nis,tip,durum,kaynak,fomo,rakip,kreatif,breakeven,fiyat,alis_fiyat,notlar) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
...
app.put("/api/urunler/:id", (req, res) => {
  const { urun,link,gorsel,nis,tip,durum,kaynak,fomo,rakip,kreatif,breakeven,fiyat,alis_fiyat,notlar,rakip_magazalar,kullanici_id } = req.body;
  db.prepare(`UPDATE urunler SET urun=?,link=?,gorsel=?,nis=?,tip=?,durum=?,kaynak=?,fomo=?,rakip=?,kreatif=?,breakeven=?,fiyat=?,alis_fiyat=?,notlar=?,guncelleme=CURRENT_TIMESTAMP WHERE id=?`)
...
app.post("/api/urunler/:id/analiz", async (req, res) => {
  ...
  const rakipler = (u.rakip_magazalar||[]).map(r => {
    const ulkeler = typeof r.ulkeler==="string" ? JSON.parse(r.ulkeler||"[]") : r.ulkeler||[];
    return `- ${r.url||"?"} (Pazarlar: ${ulkeler.join(", ")||"belirtilmemiş"})${r.notlar?" — "+r.notlar:""}`;
  }).join("\n");

  const gorselSayisi = (u.gorseller||[]).length;

  const prompt = `Sen bir dropshipping ürün analistisin. Aşağıdaki ürünü analiz et ve net, pratik bir değerlendirme yaz. Türkçe yaz. Madde madde, kısa ve öz ol.

ÜRÜN BİLGİLERİ:
- Ürün Adı: ${u.urun||"—"}
- Niş: ${u.nis||"—"}
- Ürün Tipi: ${u.tip||"—"}
- FOMO (Fırsat Kaçırma Hissi): ${u.fomo||"—"}
- Rakip Piyasada Süre: ${u.rakip||"—"}
- Kreatif Durumu: ${u.kreatif||"—"}
- 1:6 Başa Baş Noktası: ${u.breakeven||"—"}
- Satış Fiyatı Aralığı: ${u.fiyat||"—"}
- Alış Fiyatı: ${u.alis_fiyat ? "₺"+u.alis_fiyat : "—"}
- Mevcut Durum: ${u.durum||"—"}
- Tedarik Kaynağı: ${u.kaynak||"—"}
- Yüklenen Analiz Görseli Sayısı: ${gorselSayisi}
- Notlar: ${u.notlar||"—"}
- Rakip Mağazalar:\n${rakipler||"Belirtilmemiş"}

Şu başlıkları kullanarak analiz et:
...
🌍 EN İYİ PAZAR
- Hangi ülke/pazar daha uygun ve neden (1-2 cümle)
...
app.get("/api/export/csv", (req, res) => {
  ...
  const hdr = ["ID","Ürün","Link","Niş","Tip","Durum","Kaynak","FOMO","Rakip","Kreatif","1:6","Fiyat","Alış","Notlar","Oluşturma"];
  const csv = [hdr,...rows.map(u=>[u.id,u.urun,u.link,u.nis,u.tip,u.durum,u.kaynak,u.fomo,u.rakip,u.kreatif,u.breakeven,u.fiyat,u.alis_fiyat,u.notlar,u.olusturma])]
